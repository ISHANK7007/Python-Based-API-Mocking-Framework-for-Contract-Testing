...
class ContractVersionManager:
    ...
    def discover_contracts(self, directory: str) -> None:
        """
        Scan the directory for versioned YAML contract files and build the registry.
        """
        ...
        for file_path in glob.glob(pattern):
            try:
                ...
                continue  # Skip unparseable versioned filenames

        for version_list in self.versions.values():
            version_list.sort()  # Assumes ContractVersion implements __lt__
    ...
    def get_version(self, contract_name: str, version_str: str) -> Optional[ContractVersion]:
        """
        Retrieve a specific version from the registry using 'major.minor.patch'.
        """
    ...
    def load_contract(self, version: ContractVersion) -> ContractEntry:
        """
        Load and cache the contract entry from a given version file.
        """
    ...
    def compare_versions(self, contract_name: str, version1_str: str, version2_str: str) -> dict:
        """
        Compare two contract versions and identify added, removed, and modified routes.
        """
    ...
    def _identify_route_changes(self, route1, route2) -> dict:
        """
        Identify field-level differences between two matching routes.
        """
        changes = {}
    ...
    def build_comparison_baseline(self, contract_name: str) -> dict:
        """
        Build a sequential diff report of all versions for a given contract.
        """
        versions = self.versions.get(contract_name, [])
        if not versions:
